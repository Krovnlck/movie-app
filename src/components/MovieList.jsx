...
const MovieList = ({ movies, genres = [], onRate, showRating = false, loading = false, error = null }) => {
  ...
  const handleRateChange = async (movieId, value) => {
    try {
      ...
      setRatings(prev => ({ ...prev, [movieId]: prev[movieId] }));
      throw error;
    }
  };
              ...
              <div className="movie-rating-wrapper">
                ...
                {(ratings[movie.id] || movie.rating) && (
                  <span className="movie-rating-value">
                    {ratings[movie.id] || movie.rating}/10
                  </span>
                )}
              </div>
              {showRating && movie.rating && !ratings[movie.id] && (
                <div className="movie-previous-rating">
                  Your previous rating: {movie.rating}
                </div>
              )}
            </div>
          </div>
